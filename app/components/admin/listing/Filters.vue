...
const props = defineProps<{
  modelValue: IAdminAdsFilters;
  visible: boolean;
  isAdmin: boolean;
}>();

const emit = defineEmits<{
  "update:modelValue": [value: IAdminAdsFilters];
  apply: [];
  reset: [];
}>();
...
const statusOptions = computed(() => {
  const allStatuses = [
    ...
    { value: ListingStatus.REJECTED, label: "Отклонённые" },
    { value: ListingStatus.DRAFT, label: "Черновики" },
    { value: ListingStatus.SOLD, label: "Проданные" },
...
const sortOptions = [
  ...
  { value: "views_count", label: "Просмотрам" },
];
